    ...
    : IRequestHandler<EvaluateGoalsCommand, ErrorOr<EvaluateGoalsResult>>
{
    ...
    {
        // 1. Authenticate + load the owned patient (see PatientAccess). The
        //    patient is reused below for category resolution, LDL routing
        //    (HasAscvd), BMI (HeightCm), and eGFR (DateOfBirth + Gender) — its
        //    lifetime spans the rest of the handler.
