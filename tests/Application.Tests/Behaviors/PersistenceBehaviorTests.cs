...
public class PersistenceBehaviorTests
{
    ...
    private readonly PersistenceBehavior<FakeTransactionalCommand, ErrorOr<int>> _behavior;

    public PersistenceBehaviorTests()
    {
        _behavior = new PersistenceBehavior<FakeTransactionalCommand, ErrorOr<int>>(_unitOfWork);
    }
    ...
    public async Task Handle_WhenHandlerThrows_DoesNotSaveChangesAndPropagates()
    {
        ...
        Func<Task> act = () => _behavior.Handle(new FakeTransactionalCommand(), next, CancellationToken.None);
